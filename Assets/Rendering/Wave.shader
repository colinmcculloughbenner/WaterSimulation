Shader "Custom/Wave" {
    Properties{
		...
		_WaterLighteningThreshold ("Water Lightening Threshold", float) = 0.7
    	_WaterDarkeningThreshold ("Water Darkening Threshold", float) = 0.3
    	_VisibilityThreshold ("Visibility Threshold", float) = 0.5
    	_MaxAlphaThreshold ("Max Alpha Threshold", float) = 0.7
    	...
    	_DarkWaterColor ("Color of deepest wave troughs", Color) = (0,0,0,1)
	...
	SubShader{
		...
		Pass{
			...
			float _WaterDarkeningThreshold;
			float _VisibilityThreshold;
			float _MaxAlphaThreshold;

			fixed4 _Color;
			fixed4 _TopColor;
			fixed4 _BottomColor;
			fixed4 _WhitecapColor;
			fixed4 _DarkWaterColor;
			...
			fixed4 frag(v2f i) : SV_TARGET{
				...
				{
					col = lerp(_WhitecapColor, col, (1 - waterHeight) / (1 - _WaterLighteningThreshold));
				}

				if (waterHeight < _WaterDarkeningThreshold)
				{
					col = lerp(_DarkWaterColor, col, waterHeight / _WaterDarkeningThreshold);
				}

				const float waterDeviation = abs(waterHeight - 0.5);
				if (waterDeviation >= _VisibilityThreshold)
				{
					col.a *= (1 - (_MaxAlphaThreshold - waterDeviation) / (_MaxAlphaThreshold - _VisibilityThreshold));
				}
				else
				{
					col.a =0;
